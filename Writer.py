        ...
        for number in self.Machine_ID:
            try:
                ...
                if number <= 3:
                    ...
                    MachineIP = "10.10.10." + str(number - 3) #! May Need Fixing 
    ...
    def Challenge(self):
        for number in self.Challenge_ID:
            try:
                ...
                ChallengeObj = self.Client.get_challenge(self.Challenge_ID)
...
Parser.add_argument("-challenge","-c",metavar="",help="Use this to run writeup creation on a certain challenge",action="store",nargs="*",default=None,type=int)
Parser.add_argument("-machine","-m",metavar="",help="Use this to run writeup creation on a certain machine",action="store",nargs="*",default=None,type=int)
Args = Parser.parse_args()

if __name__ == "__main__":
    Password = getpass(f"\033[93mInput your Password > \033[0m") # GetPass Securely 
    Client = HTBClient(email="",password=Password)
    print(f"[*] Authenticated as {green(Client.user.name)}")
    print(f"[*] Processing {green(len(Args.challenge))} Challenges and {green(len(Args.machine))} Machines")
    Writeup(Args,Client)
